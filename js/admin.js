...
(function() {
  ...
  window.showToast = function(title, message, type = 'success') {
    ...
    toast.innerHTML = `
      <div class="admin-toast-icon ${type}">${icons[type] || 'ℹ'}</div>
  ...
  function renderOrders() {
    ...
    tbody.innerHTML = filtered.map(o => {
      ...
      const prodNames = Array.isArray(prods) ? prods.map(p => p.name || p.nombre || 'Producto').join(', ') : '';
      return `
        <tr>
          <td><strong style="color:var(--admin-text)">${o.id}</strong></td>
          <td>${o.client_name}</td>
  ...
  function renderDeliveries() {
    ...
    let alertHtml = '';
    if (delayed.length > 0) {
      alertHtml += `<div class="admin-alert admin-alert-warning"><span class="admin-alert-icon">⚠️</span><span class="admin-alert-text">${delayed.length} envío(s) en tránsito</span></div>`;
    }

    const lowStock = products.filter(p => p.stock <= p.min_stock && p.stock > 0);
    if (lowStock.length > 0) {
      alertHtml += `<div class="admin-alert admin-alert-danger"><span class="admin-alert-icon">📦</span><span class="admin-alert-text">${lowStock.length} producto(s) con stock bajo</span></div>`;
    }
  ...
  function renderInventory() {
    ...
    if (outOfStock.length > 0) {
      html += `<div class="admin-alert admin-alert-danger"><span class="admin-alert-icon">🚨</span><span class="admin-alert-text">${outOfStock.length} producto(s) agotado(s): ${outOfStock.map(p => p.name).join(', ')}</span></div>`;
    }
    if (lowStock.length > 0) {
      html += `<div class="admin-alert admin-alert-warning"><span class="admin-alert-icon">⚠️</span><span class="admin-alert-text">${lowStock.length} producto(s) con stock bajo: ${lowStock.map(p => `${p.name} (${p.stock})`).join(', ')}</span></div>`;
    }
  ...
  function renderNotifications() {
    ...
    list.innerHTML = notifications.map(n => {
      const icons = { order: '📦', stock: '⚠️', appointment: '📅', payment: '💳', delivery: '🚚' };
